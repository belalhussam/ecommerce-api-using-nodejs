class ApiFeatures {
  ...
  filter() {
    ...
    qeuryStr = qeuryStr.replace(/\b(gte|gt|lte|lt)\b/g, (match) => `$${match}`);
  ...
  search(modelName) {
    if (this.quertString.keyword) {
      ...
      if (modelName == "Product") {
        query.$or = [
          { title: { $regex: this.quertString.keyword, $options: "i" } },
          { description: { $regex: this.quertString.keyword, $options: "i" } },
        ];
      } else {
        query = { name: { $regex: this.quertString.keyword, $options: "i" } };
      }
